...
def train(epoch, data_loader, model, optimizer, criterion):
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        #############################################################################
            # clean grad buffer for each iter
        optimizer.zero_grad()
            # model forward prediction
        out=model(data)
            #compute the chosen loss with criterion
        loss=criterion(out,target)
            # compute backward 
        loss.backward()
            # Optimiwe with the corresponding optimizer
        optimizer.step()
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
...
def validate(epoch, val_loader, model, criterion):
    ...
    for idx, (data, target) in enumerate(val_loader):
        ...
        #       HINT: torch.no_grad()                                               #
        #############################################################################
            # we just want to predict so need to compute gradients or update
        with torch.no_grad():
            out=model(data)
