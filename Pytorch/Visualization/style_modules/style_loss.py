...
class StyleLoss(nn.Module):
    def gram_matrix(self, features, normalize=True):
        ...
        # Dot product in 3D tensor
        gram = torch.bmm(features_c, features_c_tp)
        #gram=torch.zeros(N,C,C)
        #for i in range(C):
        #    for j in range(C):
        #       product= features[:,i]*features[:,j]
        #        gram[:,i,j]=torch.sum(product,dim=(1,2))
    ...
    def forward(self, feats, style_layers, style_targets, style_weights):
        ...
        for i in range(len(style_layers)):
            ...
            gen_gram= self.gram_matrix(feats[style_layers[i]])
            L_i= style_weights[i]*torch.sum(torch.pow(source_gram-gen_gram,2))
            if i==0:
                style_loss=L_i
            else:
                style_loss+=L_i
