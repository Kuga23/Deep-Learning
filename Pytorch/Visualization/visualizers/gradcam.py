...
class CustomReLU(TorchFunc):
    ...
    def backward(self, y):
        ...
        ##############################################################################
        inp,out= self.saved_tensors
        fl_Rl1 = torch.addcmul(torch.zeros(inp.size()),(inp > 0).type_as(inp),(y > 0).type_as(y))
        output=fl_Rl1*y
        return output
        ##############################################################################
        #                             END OF YOUR CODE                               #
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        predict,indx= torch.max(out,dim=1)
        predict.backward(y_tensor)

        grad_X= (X_tensor.grad.data).detach().numpy()
            #swap dim to ajust shape of the matrix
        grad_X= np.einsum('klij->kijl', grad_X)
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        #for i in range(len(y_tensor)):
            #predict[i].backward(y_tensor)

        N, C, H, W = self.gradient_value.shape
        avg_pool = torch.nn.AvgPool2d((H,W))
        #alpha_i = avg_pool(self.gradient_value)
    
        alpha_i= torch.sum(self.gradient_value,dim=(2,3))/(H*W)
        relu = torch.nn.ReLU()        
        sum_channel = torch.zeros(N,H,W)
        for i in range(N):
            for j in range(C):
                sum_channel[i]+= self.activation_value[i,j]*alpha_i[i,j]
        cam= relu(sum_channel)
