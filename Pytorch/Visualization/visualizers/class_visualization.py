...
class ClassVisualization:
    ...
    def create_class_visualization(self, target_y, class_names, model, dtype, **kwargs):
        ...
        for t in range(num_iterations):
            ...
            ########################################################################
            out= model(img_var)
            target_score=out[:,target_y]
            I_=target_score-l2_reg*torch.norm(img_var)
            I_.backward()

            grad_input= img_var.grad.data
            dX= (learning_rate*grad_input)/torch.norm(grad_input)
            img_var.data+=dX.data
            img_var.grad= None
