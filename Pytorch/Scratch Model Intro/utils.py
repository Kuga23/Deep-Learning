...
def load_mnist_trainval():
    ...
    train_label = label[:int(0.80*len(data))]
    val_data = data[int(0.80*len(data)):]
    val_label = label[int(0.80*len(data)):]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle==True:
        concatene= list(zip(data,label))
        random.shuffle(concatene)
        data,label=list(zip(*concatene))
        

    # batched data/label are both list then convert to numpy
    # cause use of shape
    nb_batch= int(len(data)/batch_size) 
    batched_data=[]
    batched_label=[]
    for i in range (nb_batch) :
        batch_d=data[i*batch_size:(i+1)*batch_size] 
        batch_l=label[i*batch_size:(i+1)*batch_size]
